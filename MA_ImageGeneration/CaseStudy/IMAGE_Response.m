...
respone = load('Response_Y_wg_1.mat');
...
Y_wg = reshape(exp(2.76+0.37*mean_response(:,4)), [10, 10]);
Z = reshape((mean_response(:,7)), [10, 10]);
Z_up = reshape((mean_response(:,7)+std_response(:,7)), [10, 10]);
Z_down = reshape((mean_response(:,7)-std_response(:,7)), [10, 10]);


% Create the surface plot
figure;
surf(X, Y_wg, Z,'EdgeColor','interp','FaceColor','interp','FaceAlpha',0.9);
hold on 
surf(X, Y_wg, Z_up,'EdgeColor','k','FaceColor','none','FaceAlpha',0.5);
surf(X, Y_wg, Z_down,'EdgeColor','k','FaceColor','none','FaceAlpha',0.5);
xlabel('$M_{L}$', 'Interpreter', 'latex');
ylabel('$\omega_{g}/2\pi$', 'Interpreter', 'latex');
zlabel('$ln(v_{max}) (mm/s)$', 'Interpreter', 'latex');
xlim([0,2.5]);
ylim([7.5,27.5224]);
colorbar;
...
%zlabel('$v_{max} (mm/s)$', 'Interpreter', 'latex');
%colorbar;
%view(2)
...
[X_,Y_] = meshgrid(M_,R_);
flattened = [X_(:), Y_(:)];
all_data = zeros(length(flattened),4);

%for i = 1:length(flattened)
%    Pesuedo_M = flattened(i,1).*ones(num_DOE,1);%exp(-0.378+ 0.53*X.X(:,1));
%    Pesuedo_R = flattened(i,2).*ones(num_DOE,1);%sqrt(power(8+ 7.5*X.X(:,2),2) + power(8+ 7.5*X.X(:,3),2));
%    Pesuedo_Wg = exp(2.76+ 0.37*mean(X.X(:,4))).*ones(num_DOE,1);
%    X_rand = fns_generateTable_Scenario(LME_lnPGA,LME_lnWc,Pesuedo_M,Pesuedo_R,Pesuedo_Wg,0.3);
%    Y_sampe_point = SDOF_simulation(X_rand,dir,valid_point);
%    all_data(i,:) = [flattened(i,1),flattened(i,2),mean(Y_sampe_point),std(Y_sampe_point)];
%end
...
function Y = SDOF_simulation(X, dir_ANSYS,fixpoint)
       ...
       for i = 1:10
           ...
           disp(['     Parameter: PGA = ',num2str(PGA(i))])
