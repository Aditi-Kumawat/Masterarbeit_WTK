...
DOE_path = "C:\Users\v196m\Desktop\master_project\Masterarbeit\StochasticPCE\InputData\X5_SBAGM_VAL_1000.mat";
X = load(DOE_path);
dir = 'Z';
valid_point = 200;
%output_type = "rand";
...
if strcmp(output_type,"rand")
    ...
    Pesuedo_R = 8+ 7.5*X.X(:,2);
    Pesuedo_Wg = exp(2.76+ 0.37*X.X(:,3));
    X_rand = fns_generateTable_Scenario(LME_lnPGA,LME_lnWc,Pesuedo_M,Pesuedo_R,Pesuedo_Wg,0.3);
    X_rand.Wb = 12+1*X.X(:,4);
    X_rand.beta = exp(-3.2+0.1*X.X(:,5));
    Y = SDOF_simulation(X_rand, output_type, valid_point);
...
elseif strcmp(output_type,"fix")
    ...
    Pesuedo_R = 8+ 7.5*Valid_point_1(:,2);
    ...
    X_valid.Wb = 12+1*Valid_point_1(:,4);
    X_valid.beta = exp(-3.2+0.1*Valid_point_1(:,5));

    Y = SDOF_simulation(X_valid, output_type, valid_point);
end



function Y = SDOF_simulation(X, X_type, valid_point)
    ...
    if strcmp(X_type, "rand")
        ...
        for i = 1:length(X.lnPGA)
            ...
            Omega = 2*pi*f;
            FRF = (power(Omega,2))./(-power(Omega,2)+ 2*1i*Omega*DR(i)*Wn(i)+ power(Wn(i),2));
        ...
        file_name = sprintf('TESTING5_PRED_SDOF_SBAGM_Y_rand_%d.mat', length(X.lnPGA));
        save(file_name,'Y','-mat');


    elseif strcmp(X_type, "fix")
        ...
        file_name = sprintf('TESTING5_PRED_SDOF_SBAGM_Y_fix_%d_num_%d.mat',valid_point,length(X.lnPGA));
        save(file_name,'Y','-mat');
    end
