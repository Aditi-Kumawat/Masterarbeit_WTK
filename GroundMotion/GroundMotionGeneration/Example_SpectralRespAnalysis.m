...
if strcmp(output_type,"rand")
    ...
    X_rand = fns_generateTable_Scenario(LME_lnPGA,LME_lnWc,Pesuedo_M,Pesuedo_R,Pesuedo_Wg,0.39);
    X_rand.Wb = 12+1*X.X(:,5);
    X_rand.beta = exp(-3.2+0.1*X.X(:,6));
    Y = SDOF_simulation(X_rand,Learning_type,'rand',[]);
...
elseif strcmp(output_type,"fix")
    ...
    Pesuedo_M = Info_matrix(fix_p,1).*ones(num_DOE,1);%exp(-0.378+ 0.53*Valid_point_1(:,1));
    %Pesuedo_R = 8.5+ 2.2*Valid_point_1(:,2);
    Pesuedo_R = Info_matrix(fix_p,3).*ones(num_DOE,1);%sqrt(power(8+ 7.5*Valid_point_1(:,2),2) + power(8+ 7.5*Valid_point_1(:,3),2));
    Pesuedo_Wg = Info_matrix(fix_p,5).*ones(num_DOE,1);%exp(2.76+ 0.37*Valid_point_1(:,4));
    

    data_t = fns_import_time_data(Record_info{fix_p,1}{1},'txt',[]);
    [b, a] = butter(2, 30/(200/2));  % Normalized cutoff frequency
    y = filtfilt(b, a, double(data_t.ampl));
    
    data_record = data_t;
    data_record.ampl = y;

    GMG = cls_GM_generator([],data_t, 100);
    [GM,FRF_info,Time_info,GM_info] = GMG.generateTimeNonStaPesudoGMbyFit("Hu_S0",[150,1,0.5,GMG.PGA],[]);
    X_valid = fns_generateTable_Scenario(LME_lnPGA,LME_lnWc,Pesuedo_M,Pesuedo_R,Pesuedo_Wg,0.2254);
    X_valid.Wb = 12+1*Valid_point_1(:,5);
    X_valid.beta = FRF_info(2).*ones(num_DOE,1); %exp(-3.2+0.1*Valid_point_1(:,6));

    Y = SDOF_simulation(X_valid,Learning_type,'fix',fix_p,data_record);

end


function Y = SDOF_simulation(X, learning_type, output_type,fixpoint,fitting_GM)
 ...
 %MODIFY        
       ...
       elseif strcmp(output_type,"fix")
           ...
           for i = 1:num_repeat
               ...
               GM_params = [W_g(i), beta(i) ,W_c_ratio(i)];
               [Time, Acc, Vel, ~] = fns_generateGM_Params([], [], 100 ,"Hu_S0", GM_params, [0.5, 1 ,4, 10], PGA(i));
               [T_p,Spa_p,Spv_p,Sd_p] = fns_response_spectra(Time(3)-Time(2),Acc,5,9.81,10);
               length(T_p)
               Acc_pesuedo(i,:) = transpose(Spa_p);
               Disp_pesuedo(i,:) = transpose(Spv_p);
               plot(Time,Acc);

           end
           hold off
           
           [T_o,Spa_o,Spv_o,Sd_o] = fns_response_spectra(fitting_GM.time(3)-fitting_GM.time(2),1.0*fitting_GM.ampl,5,9.81,10);

           %% Plot Spectra
            figure;
            subplot(2,1,1)
            %figure('Name','Spectral Displacement','NumberTitle','off')
            %semilogx(T_p,Sd_p,'LineWidth',2., Color="#0072BD")
            %hold on 
            
            semilogx(T_o,Spv_o,'LineWidth',1., Color="#0072BD")
            hold on 
            semilogx(T_p,Disp_pesuedo,'LineWidth',1., Color="#EDB120")
            semilogx(T_o,Spv_o,'LineWidth',1., Color="#0072BD")
            grid on
            legend('Recorded GM','GM from SBGMG', 'Interpreter', 'latex');
            xlabel('Period (sec)', 'Interpreter', 'latex');
            ylabel('Spectrum velocity (m/s)', 'Interpreter', 'latex');
            title('Velocity Spectrum', 'Interpreter', 'latex')
            
            
            subplot(2,1,2)
            %figure('Name','Pseudo Acceleration Spectrum','NumberTitle','off')
            semilogx(T_o,Spa_o,'LineWidth',1., Color="#0072BD")
            hold on 
            semilogx(T_p,Acc_pesuedo,'LineWidth',1., Color="#EDB120")
            %hold on 
            semilogx(T_o,Spa_o,'LineWidth',1., Color="#0072BD")
            grid on
            legend('Recorded GM','GM from SBGMG', 'Interpreter', 'latex');
            xlabel('Period (sec)', 'Interpreter', 'latex');
            ylabel('Spectrum acceleration (g)', 'Interpreter', 'latex');
            title('Acceleration Spectrum', 'Interpreter', 'latex');
       end
